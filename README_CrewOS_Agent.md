...
1. Clone/pull repositori, lalu install dependensi:
   ```
   pip install -r requirements.txt
   ```
2. Salin `envexample.md` menjadi `.env` dan isi tiga variabel berikut:
   ```
   ...
   ```
3. Jalankan **tiga proses** pada terminal terpisah (urutan tidak wajib, tapi Orchestrator butuh dua service lain untuk fitur penuh):
   ```
   ...
   ```
4. Buka `http://localhost:8000/app` — antarmuka chat yang sudah terhubung ke kedua agen spesialis.
